...
import { C, fmt, fmtShort, getCat, Transaction, CATS_EXPENSE, CATS_INCOME, PAYMENTS, FIXED_EXPENSES_DEFAULT } from './Constants';
import { Icon, CatIcon } from './Icons';

function WhoTag({ who }: { who: string }) {
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
      who === 'husband' ? 'bg-[#ebf2ff] text-[#3182f6]' : 'bg-[#fceeff] text-[#c759d6]'
    }`}>
      {who === 'husband' ? '남편' : '아내'}
    </span>
  );
...
export function TxRow({ tx, last, onClick }: { tx: Transaction, last: boolean, onClick?: (tx:Transaction)=>void }) {
  const cat = getCat(tx.type, tx.category);
  return (
    <div onClick={() => onClick?.(tx)} className={`flex items-center gap-3 py-3.5 ${!last ? 'border-b border-[var(--border)]' : ''} ${onClick ? 'cursor-pointer hover:bg-gray-50 active:scale-[0.98] transition-all -mx-2 px-2 rounded-xl' : ''}`}>
      <CatIcon cat={cat} />
      <div className="flex-1 min-w-0">
        <div className="text-[15px] font-bold text-[var(--foreground)] mb-1 truncate">{tx.category}</div>
        <div className="flex flex-wrap gap-2 items-center">
          <span className="text-[12px] text-[var(--text-muted)] truncate max-w-[140px] block">{tx.memo}</span>
          <WhoTag who={tx.who} />
        </div>
      </div>
      <div className={`text-[16px] font-bold shrink-0 ${tx.type === 'income' ? 'text-[var(--primary)]' : 'text-[var(--foreground)]'}`}>
...
export function HomeScreen({ transactions, onUpload, assets, onAssetsChange }: { 
  ...
  const startEdit = (asset: AssetItem) => { setEditingId(asset.id); setEditVal(String(asset.amount)); };
...
export function CalendarScreen({ transactions, onRowClick, onDateSelect }: { transactions: Transaction[], onRowClick?: (tx:Transaction)=>void, onDateSelect?: (date:string)=>void }) {
    ...
    <div className="flex-1 overflow-y-auto bg-[var(--background)] pb-24 animate-[fadeIn_0.3s_ease]">
      ...
      <div className="mt-4 mx-4 sm:mx-6 rounded-[28px] bg-white p-5 pb-3 shadow-sm">
        ...
        {selTxs.length===0
          ...
          : selTxs.map((tx,i)=><TxRow key={tx.id} tx={tx} onClick={onRowClick} last={i===selTxs.length-1} />)
        }
      </div>
    </div>
  );
...
export function LedgerScreen({ transactions, onRowClick }: { transactions: Transaction[], onRowClick?: (tx:Transaction)=>void }) {
  const [currentDate, setCurrentDate] = useState(() => {
    const dates = transactions.map(t=>t.date).sort();
    return dates.length > 0 ? new Date(dates[dates.length-1]) : new Date();
  });
      ...
      {Object.entries(groups).map(([date,txs])=>{
        ...
        return (
          <div key={date} className="mx-4 sm:mx-6 rounded-[28px] bg-white p-5 pb-3 shadow-sm mb-4">
            ...
            {txs.map((tx,i)=><TxRow key={tx.id} tx={tx} onClick={onRowClick} last={i===txs.length-1} />)}
          </div>
        );
      })}
    </div>
...
export function FixedScreen({ onApply, showAddFixed, onCloseAddFixed }: { onApply: (tx: Transaction) => void, showAddFixed?: boolean, onCloseAddFixed?: () => void }) {
  ...
  const [newCategory, setNewCategory] = useState('생활');
  ...
  function addNew() {
    ...
    setNewLabel(''); setNewAmount(''); setNewCategory('생활');
    onCloseAddFixed?.();
  }
  function apply() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[var(--background)] pb-24 animate-[fadeIn_0.3s_ease]">
      ...
      <div className="mx-4 sm:mx-6 flex flex-col gap-3 mb-4">
        ...
        {showAddFixed && (
          <div className="rounded-[20px] bg-white p-4 shadow-md border-2 border-[var(--primary)] animate-[fadeIn_0.2s_ease]">
            <div className="text-[14px] font-bold text-[var(--primary)] mb-3">✨ 새 고정비 추가</div>
            <input value={newLabel} onChange={e=>setNewLabel(e.target.value)} placeholder="항목명 (예: 넷플릭스)" className="w-full text-[14px] font-bold p-2.5 rounded-xl border border-gray-200 outline-none bg-gray-50 mb-2" />
            <input type="number" value={newAmount} onChange={e=>setNewAmount(e.target.value)} placeholder="금액 (예: 17000)" className="w-full text-[14px] font-bold p-2.5 rounded-xl border border-gray-200 outline-none bg-gray-50 mb-2" />
            <select value={newCategory} onChange={e=>setNewCategory(e.target.value)} className="w-full text-[13px] font-bold p-2.5 rounded-xl border border-gray-200 outline-none bg-gray-50 mb-3 text-[var(--foreground)]">
              {['생활','주거','통신','보험','교육','구독','교통','기타'].map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <div className="flex gap-2">
              <button onClick={addNew} className="flex-1 py-2.5 rounded-xl text-white text-[13px] font-bold" style={{background:'var(--primary-gradient)'}}>추가하기</button>
              <button onClick={onCloseAddFixed} className="px-4 py-2.5 rounded-xl bg-gray-100 text-[13px] font-bold text-gray-500">취소</button>
            </div>
          </div>
        )}
            ...
            {editIdx === item.id ? (
              ...
              <>
                ...
                <div className="flex items-center gap-3 pr-12">
                  <div className="w-11 h-11 rounded-[12px] flex items-center justify-center shrink-0" style={{ background: item.bg }}>
                    <Icon name={item.icon} size={20} color={item.fg} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className={`text-[14px] font-bold mb-0.5 ${item.active?'text-[var(--foreground)]':'text-gray-400'}`}>{item.label}</div>
                    <div className="text-[11px] text-gray-400">{item.category} · 매월 자동</div>
                  </div>
                </div>
...
export function TxFormModal({ 
  ...
  const accent = type==='expense' ? '#191f28' : '#8b5cf6'; 

  function num(n:string) {
    if(n==='del') setAmount(p=>p.slice(0,-1));
    else if(amount.length<10) setAmount(p=>p+n);
  }
  function save() {
    ...
    <div className="fixed inset-0 z-[200] bg-black/40 backdrop-blur-sm flex flex-col justify-end animate-[fadeIn_0.2s_ease]" onClick={e=>{ if(e.target===e.currentTarget) onClose(); }}>
      <div className="bg-white rounded-t-[32px] w-full sm:max-w-[440px] mx-auto overflow-hidden flex flex-col max-h-[94vh] shadow-2xl animate-[slideUp_0.3s_ease]">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar px-5 pt-3 pb-4">
          ...
          <input value={memo} onChange={e=>setMemo(e.target.value)} placeholder="어디서 썼나요? (메모)"
          ...
          <button onClick={save} disabled={!amount||!category}
            className={`w-full py-4.5 rounded-[18px] text-[16px] font-bold transition-all ${(!amount||!category)?'bg-gray-200 text-gray-400':'text-white shadow-lg shadow-[var(--primary)]/30 active:scale-95'}`}
            style={{ background: (!amount||!category) ? undefined : 'var(--primary-gradient)' }}>
            {isEdit ? '수정 완료' : '장부에 기록'}
          </button>
        </div>
      </div>
